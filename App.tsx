...
type View = 'login' | 'dashboard' | 'calendar' | 'list' | 'patients' | 'new-appointment' | 'config' | 'admin' | 'public-booking';
...
const App: React.FC = () => {
  ...
  const handleCreateAppointment = async (newAppointment: Appointment) => {
    // 1. Guardar localmente de inmediato para UX fluida
    ...
    // 2. Sincronización con Odoo de la compañía que posee la cita
    const targetCompanyId = newAppointment.company_id;
    const currentCompany = config.companies.find(c => c.id === targetCompanyId) || publicCompany;
    ...
    if (currentCompany?.odoo.url) {
      try {
        ...
        if (authed) {
          // Crear Partner y Venta en Odoo
          const partnerId = await odoo.findOrCreatePartner(newAppointment.patient);
          const products = await odoo.getMedicalProducts();
          const targetProduct = products.find(p => p.name.toLowerCase().includes(newAppointment.tipo)) || products[0]; 
          
          if (targetProduct) {
            const saleOrderId = await odoo.createSaleOrder(newAppointment, partnerId, targetProduct.id);
            
            // Actualizar registro local con IDs de Odoo
            setAppointments(prev => {
              const res = prev.map(app => 
                app.id === newAppointment.id 
                ? { ...app, odoo_partner_id: partnerId, odoo_sale_order_id: saleOrderId } 
                : app
              );
              persist('appointments', res);
              return res;
            });
            console.log(`Sync Odoo exitoso: Partner ${partnerId}, Pedido ${saleOrderId}`);
          }
        }
      } catch (e) { 
        console.error(`Error de sincronización para ${currentCompany.name}:`, e); 
      }
    }
  };
    ...
    <div className="flex min-h-screen bg-[#f4f7f6]">
        ...
        <div className="p-6 flex flex-col h-full">
          <div className="flex items-center gap-4 mb-12">
            ...
            <div>
               ...
               <p className="text-[9px] text-white/40 font-bold tracking-widest mt-1">SaaS MEDICAL NETWORK</p>
            </div>
          </div>
          ...
          <nav className="space-y-2 flex-1">
            {[
              ...
              { id: 'list', label: 'Citas Hoy', icon: ClipboardList },
              { id: 'patients', label: 'Pacientes e Historial', icon: Users },
              { id: 'new-appointment', label: 'Nueva Cita Manual', icon: UserPlus },
              { id: 'admin', label: 'Sedes y Doctores', icon: Building2 },
              { id: 'config', label: 'Odoo & SaaS Config', icon: ShieldCheck },
            ].map(item => (
              <button
                key={item.id}
          ...
          <button onClick={() => setActiveView('login')} className="flex items-center gap-3 px-4 py-3 text-white/40 hover:text-red-400 transition-colors mt-auto font-medium text-sm">
      ...
      <main className="flex-1 overflow-auto">
        <header className="sticky top-0 bg-white/60 backdrop-blur-xl border-b border-slate-200 px-8 py-5 flex justify-between items-center z-30">
          ...
          <div className="flex items-center gap-6 ml-auto">
            <div className="text-right hidden sm:block">
              ...
              <p className="text-[10px] text-slate-400 mt-1 font-bold">Modo Administrador</p>
            </div>
            <div className="w-11 h-11 rounded-2xl bg-slate-100 flex items-center justify-center font-bold text-[#1e3050]">
        ...
        <div className="p-8 max-w-[1400px] mx-auto">
          ...
          {activeView === 'list' && <AppointmentList appointments={appointments} onUpdateStatus={(id, status) => {
            const updated = appointments.map(a => a.id === id ? {...a, estado: status} : a);
            setAppointments(updated);
            persist('appointments', updated);
          }} />}
          {activeView === 'patients' && <PatientList appointments={appointments} />}
          {activeView === 'new-appointment' && <AppointmentForm appointments={appointments} sedes={sedes} doctors={doctors} onSave={(app) => handleCreateAppointment({...app, company_id: config.activeCompanyId})} />}
